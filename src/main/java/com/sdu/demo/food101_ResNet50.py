...
def load_model(weights_path, shape):
    ...
    model_final = tf.keras.models.load_model('my_model')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', help='path of the saved model', dest='model_path',
                        type=str, default='./saved_models_food101/weights.epoch-02-val_loss-2.95.hdf5')
    ...
    if args.mode == 'train':
        X_train, X_test = setup_generator('./data/food-101/train', './data/food-101/test', args.batch_size, shape[:2])
        ...
        # add a callback
        callbacks.append(ModelCheckpoint(filepath='./weights.epoch-{epoch:02d}-val_loss-{val_loss:.2f}.hdf5',
        ...
        # trained_model.save('my_model',save_format='tf')
        image = load_image(args.image_path, shape[:2])
        preds = trained_model.predict(image)
        classes = get_classes('./data/classes.txt')
